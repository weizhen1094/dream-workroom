...
void BVHierarchy::BuildTopDown(int _depth, std::unique_ptr<TreeNode> const&  _node, std::vector<entity>& _ents)
{
	...
	ECS.registry().get<BVList>(m_BVHTreeEnt).push_back(_node->boundingVolume);

	// set flags for bvh level visibility
	if (m_VisibilityFlags.size() <= _depth) { m_VisibilityFlags.push_back(std::vector<bool*>()); }
	m_VisibilityFlags[_depth].push_back(&_node->boundingVolume->isActive); _node->boundingVolume->isActive = true;
	_node->boundingVolume->depth = _depth;
	...
	else // split
	{
		...
		BuildTopDown(_depth + 1, _node->pLeft, right);
	}
}
...
unsigned BVHierarchy::PartitionObjects(std::vector<entity>& _ents, int _axis)
{
	...
	case TOP_DOWN_K_EVEN_SPLIT_SINGLE_AXIS: 
	{
		...
		for (unsigned i = 0; i < _ents.size(); i + k)
		{
...
void BVHierarchy::BuildBottomUp(std::vector<entity>& _ents)
{
	...
	while (availList.size() > 1)
	{
		...
		// compute bv
		ComputeParentBVTreeNode(parent, { parent->pLeft->boundingVolume, parent->pRight->boundingVolume });
	...
	while (!queue.empty())
	{
		...
		for (unsigned i = 0; i < qSize; ++i)
		{
			...
			ECS.registry().get<BVList>(m_BVHTreeEnt).push_back(node->boundingVolume);
			lvFlags.push_back(&node->boundingVolume->isActive);
			node->boundingVolume->isActive = true;
			node->boundingVolume->depth = depth;
...
std::pair<unsigned, unsigned> BVHierarchy::FindNodesToMerge(std::vector<std::unique_ptr<TreeNode>>& _availList)
{
	...
	for (unsigned i = 0; i < _availList.size(); ++i)
	{
		for (unsigned j = i + 1; j < _availList.size(); ++j)
		{
			...
			{
				distCost = GetDistanceCost(_availList[i]->boundingVolume, _availList[j]->boundingVolume);
			}
			if (m_Config.mergeHeuristics[BTM_UP_MIN_CHILD_VOL])
			{
				volCost = GetVolumeCost(_availList[i]->boundingVolume, _availList[j]->boundingVolume);
			}
			if (m_Config.mergeHeuristics[BTM_UP_MIN_CHILD_SA])
			{
				saCost = GetSACost({ _availList[i]->boundingVolume }, { _availList[j]->boundingVolume });
			}
...
void BVHierarchy::ComputeParentBVTreeNode(std::unique_ptr<TreeNode> const& _node, BVList const& _bv)
{
	switch (m_Config.type)
	{
		case AABB:
		{
			...
			_node->boundingVolume = std::make_shared<Aabb>(Aabb(MergeAabbs(tmp)));
		}
		break;
		case BSPHERE_PCA:
		{
			std::vector<std::shared_ptr<Sphere>> tmp; for (auto bv : _bv) { tmp.push_back(std::dynamic_pointer_cast<Sphere>(bv)); }
			_node->boundingVolume = std::make_shared<Sphere>(Sphere(MergeSpheres(tmp)));
			_node->boundingVolume->type = BSPHERE_PCA;
		}
		break;
