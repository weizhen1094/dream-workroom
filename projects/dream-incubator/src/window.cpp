...
unsigned Window::m_CurrentWindowIdx = 0;
...
bool Window::InitWindow()
{
	...
	m_pWindows.push_back(this); m_CurrentWindowIdx = m_pWindows.size() - 1;
...
void Window::OnMouseScrollInput(GLFWwindow* _pWin, double _xOffset, double _yOffset)
{
	(void)_pWin;
#ifdef _DEBUG
	//std::cout << "Mouse scroll wheel offset: (" << _xOffset << ", " << _yOffset << ")" << std::endl;
#endif
...
Window* Window::GetCurrentWindowPtr() { return m_pWindows[m_CurrentWindowIdx]; }
double Window::GetCurrentWindowDt()   { return m_pWindows[m_CurrentWindowIdx]->GetDeltaTime(); }
