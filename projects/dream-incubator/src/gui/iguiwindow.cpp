...
void DisplayTxtFileGUI::Render()
{
    ...
    if (ImGui::Begin(m_GUIWindowTitle.c_str(), &m_IsWindowOpen, m_GUIWindowFlags))
    {
        ...
        for (auto const& line : m_TxtFile) 
        {  
            bool isHeader = line.find("#") != std::string::npos;
            if (isHeader) { ImGui::PushStyleColor(ImGuiCol_Text, (ImVec4)ImColor::HSV(0.3f, 1.0f, 1.0f)); }
            ImGui::Text(line.c_str()); 
            if (isHeader) { ImGui::PopStyleColor(1); }
        }
    }
    ImGui::End();
}
