...
bool ECSManager::IsEntityValid(entt::entity _ent) { return m_Registry.valid(_ent); }
...
{   if (m_Registry.valid(_ent)) { m_Registry.destroy(_ent); }  }
    ...
    {
        entity ent = CreateDefaultEntity();
            m_Registry.get<Transform>(ent).position = vec3(0.0f, 10.0f, 0.0f);
            m_Registry.get<Transform>(ent).rotation.z = -30.f;
    ...
    entity ECSManager::CreatePlaneEntity()
    {
        ...
        m_Registry.get<EntityName>(ent).value = "Plane";
            m_Registry.get<Transform>(ent).rotation.x = 90.f;
