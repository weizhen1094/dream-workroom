...
static const std::unordered_map<SIDE_RESULT, vec3> vfcColors =
{
    {SIDE_RESULT::INSIDE,       vec3(0.9f, 0.5f, 0.3f) },
    {SIDE_RESULT::OUTSIDE,      vec3(0.f,0.f,1.f) },
    {SIDE_RESULT::OVERLAPPING, vec3(0.75f,1.f,0.f) },
};
...
struct Aabb : BoundingVolume
{
    ...
    vec3 GetMin() const { return center - halfExtents; };
    vec3 GetMax() const { return center + halfExtents; };
    Aabb(BV_TYPE _type = AABB) : BoundingVolume(_type), center(vec3(0)), halfExtents(vec3(0.5f)) { }
    void UpdateBV(Transform& _xform) override
    {
...
struct Obb : Aabb
{
    ...
    {
        center = _xform.getMtx() * vec4(center, 1);
        halfExtents *= abs(_xform.scale);
        for (unsigned i = 0; i < 3; ++i)
        { axes[i] = mat3(_xform.getRotMtx()) * axes[i]; }
...
static std::tuple <std::vector<vec3>,Obb> CreateObbPCA(std::vector<Vertex> const& _vertices)
{
    ...
    if (eigenVectors.determinant() < 0)  { eigenVectors.col(2) = -eigenVectors.col(2); }
    for (int i = 0; i < 3; ++i)
    {
        vec3 axis { eigenVectors(0, i), eigenVectors(1, i), eigenVectors(2, i) };
        obb.axes[i] = axis;
    }
    //// gran schmidt to orthonormalize axes
    //obb.axes[0] = normalize(obb.axes[0]);
    //obb.axes[1] = obb.axes[1] - dot(obb.axes[1], obb.axes[0]) * obb.axes[0];
    //obb.axes[1] = normalize(obb.axes[1]);
    //obb.axes[2] = obb.axes[2] - dot(obb.axes[2], obb.axes[0]) * obb.axes[0];
    //obb.axes[2] = obb.axes[2] - dot(obb.axes[2], obb.axes[1]) * obb.axes[1];
    //obb.axes[2] = normalize(obb.axes[2]);
    ...
    obb.halfExtents = abs(vec3(halfExts(0), halfExts(1), halfExts(2)));
...
static std::vector<vec3> GetColors()
{
    return 
    {
    vec3(1.f,0.f,0.f),
...
static std::vector<std::shared_ptr<Aabb>> GetAABBs(std::vector<entity> const& _ents)
{
    std::vector<std::shared_ptr<Aabb>> ret;
    for (auto ent : _ents)
    {
        ...
        if (bv != bvList.end()) { ret.push_back(std::dynamic_pointer_cast<Aabb>(*bv)); }
        else { ret.push_back(std::make_shared<Aabb>()); }
    } return ret;
};

static std::pair<Aabb, Aabb> SplitAABB(Aabb const& _aabb, unsigned _axis, float splitPoint)
{
    Aabb left, right;
    vec3 leftMin = _aabb.GetMin(); vec3 rightMax = _aabb.GetMax();
    vec3 leftMax = rightMax; leftMax[_axis] = splitPoint;  
    vec3 rightMin = leftMin; rightMin[_axis] = splitPoint;

    left.center = (leftMax + leftMin) * 0.5f;
    left.halfExtents = (leftMax - leftMin) * 0.5f;

    right.center = (rightMax + rightMin) * 0.5f;
    right.halfExtents = (rightMax - rightMin) * 0.5f;
