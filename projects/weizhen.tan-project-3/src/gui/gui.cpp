...
bool GUI::InitGUI(Window* _pWin)
{
    ...
    CreateTxtFileGUI("../projects/weizhen.tan-project-3/README.txt"); // TODO: change to appropriate proj
    //CreateTxtFileGUI("../projects/project-2/README.txt"); 
    //CreateTxtFileGUI("../projects/project-3/README.txt"); 
...
void GUI::RenderMainMenuBar()
{
    if (ImGui::BeginMainMenuBar())
    {
        if (ImGui::BeginMenu("[Scenes]")) // TODO: serialization
        {
            ...
            if (ImGui::MenuItem("Sphere vs Sphere")) {}
            if (ImGui::MenuItem("AABB vs Sphere")) {}
            if (ImGui::MenuItem("Sphere vs AABB")) {}
            if (ImGui::MenuItem("AABB vs AABB")) {}
            ImGui::SeparatorText("Point-Based Tests");
            if (ImGui::MenuItem("Point vs Sphere")) {}
            if (ImGui::MenuItem("Point vs AABB")) {}
            if (ImGui::MenuItem("Point vs Triangle")) {}
            if (ImGui::MenuItem("Point vs Plane")) {}
            ImGui::SeparatorText("Ray-Based Tests");
            if (ImGui::MenuItem("Ray vs Sphere")) {}
            if (ImGui::MenuItem("Ray vs AABB")) {}
            if (ImGui::MenuItem("Ray vs Triangle")) {}
            if (ImGui::MenuItem("Ray vs Plane")) {}
            ImGui::SeparatorText("Plane-Based Tests");
            if (ImGui::MenuItem("Plane vs AABB")) {}
            if (ImGui::MenuItem("Plane vs Sphere")) {}
            ImGui::EndMenu();
        }
        ...
        {
            CreateTxtFileGUI("../projects/weizhen.tan-project-3/README.txt");
            ImGui::EndMenu();
        }
...
void GUI::DockChildWindows(unsigned& _id)
{
    ...
    if (once) {
        ...
        ImGui::DockBuilderDockWindow("Inspector##1", dock_id_right);
        ImGui::DockBuilderDockWindow("Console##2", dock_id_down);
...
{
    vec3 camPos = vec3(0.f, 5.f, 7);
    vec3 camRot = vec3(-15.f, 0.f, 0.0f);
    float near{ 0.3f }; float far{ 45 };
...
{
    vec3 camPos = vec3(0.f, 15.f, 0.0f);
    vec3 camRot = vec3(-90.f, 0.f, 0.0f);
    float near{ 0.3f }; float far{ 45 };
...
void GUI::CreateBVScene()
{
    ...
    for (auto ent : ents)
    {
        ...
        bvList.push_back(std::make_shared<Sphere>());
        bvList.back()->InitBV(mesh.GetMeshType()); bvList.back()->UpdateBV(xform);
        bvList.push_back(std::make_shared<Sphere>(Sphere(BSPHERE_Larssons)));
        bvList.back()->InitBV(mesh.GetMeshType()); bvList.back()->UpdateBV(xform);
...
void GUI::CreateBVHScene()
{
    ...
    ents.push_back(o);
    auto r = ECS.CreateMeshEntity("rhino.obj");
    ECS.registry().get<Transform>(r).position = vec3(-1.f, 0, -8.f);
    ECS.registry().get<Transform>(r).scale = vec3(0.15f);
    ents.push_back(r);
