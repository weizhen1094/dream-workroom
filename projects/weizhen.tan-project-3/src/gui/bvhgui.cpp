...
void BVHGUI::Render()
{
    ...
    if (ImGui::Begin(m_GUIWindowTitle.c_str(), &m_IsWindowOpen, m_GUIWindowFlags))
    {
        ...
        if (isTopDown)
        {
            ...
            ImGui::RadioButton("Median BV center", &e, TOP_DOWN_BV_CENTER_MEDIAN);
            if (ImGui::IsItemDeactivatedAfterEdit()) { BVH.GetConfig().splitStrategy = TOP_DOWN_BV_CENTER_MEDIAN; UpdateBVH(); }
            ImGui::SameLine();
            ImGui::RadioButton("Median BV extends", &e, TOP_DOWN_BV_EXTENT_MEDIAN);
            if (ImGui::IsItemDeactivatedAfterEdit()) { BVH.GetConfig().splitStrategy = TOP_DOWN_BV_EXTENT_MEDIAN; UpdateBVH(); }
            ImGui::SameLine();
            ImGui::RadioButton("k-even splits", &e, TOP_DOWN_K_EVEN_SPLIT_SINGLE_AXIS);
            if (ImGui::IsItemDeactivatedAfterEdit()) { BVH.GetConfig().splitStrategy = TOP_DOWN_K_EVEN_SPLIT_SINGLE_AXIS; UpdateBVH(); }
