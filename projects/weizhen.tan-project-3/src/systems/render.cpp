...
void Render::Init()
{
    ...
    if (Renderable::GetShaderPgm() == NULL)
    {
        Renderable::GetShaderPgm() = LoadShaders(
            "../projects/weizhen.tan-project-3/shaders/ColorVertexShader.vert", 
            "../projects/weizhen.tan-project-3/shaders/ColorFragmentShader.frag");
    }
    // create meshes
...
void Render::Update()
{
    ...
    viewCamera.each([](auto _ent, Transform& _xform, Camera& _cam, Framebuffer& _fb)
    {
        ...
        UseShader(Renderable::GetShaderPgm());
            ...
            static  auto _cam1 = _ent; _cam1 = _ent;
            // draw wireframe frustum of other cameras
            ECS.registry().view<Camera, Transform>().each([](auto _ent1, Camera& _cam2, Transform& _xform2)
            {
                    if (_cam1 != _ent1)
                    {
        ...
        viewMesh.each([](auto _ent,/*RenderOrder& _renderOrder,*/ Transform& _xform, Renderable& _model, Material& _mat)
            {
                ...
                UseShader(Renderable::GetShaderPgm());
                    for (auto& mesh : Renderable::GetBuffers()[_model.GetMeshType()])
                    {
                        mesh->BindVAO();
                        SetUniform(Renderable::GetShaderPgm(), "material.kAmbient", _mat.kAmbient);
                        SetUniform(Renderable::GetShaderPgm(), "material.kDiffuse", _mat.kDiffuse);
        ...
        viewBV.each([](auto _ent,/*RenderOrder& _renderOrder,*/ Transform& _xform, BVList& _bvArr)
            {
                for (auto bv : _bvArr)
                {
                    ...
                    if (hasMdl) { mdl = ECS.registry().get<Renderable>(_ent).GetMeshType(); }

                    // update xform mtx accrd to current ent xform 
                    if (_xform.isDirty)
                    {
                        if (hasMdl) { bv->InitBV(mdl); }
                        bv->UpdateBV(_xform);
                    }
                    ...
                    UseShader(Renderable::GetShaderPgm());
                        ...
                        {
                            SetUniform(Renderable::GetShaderPgm(), "material.kAmbient", BVH.GetBVHLvColors()[bv->depth]);
                            SetUniform(Renderable::GetShaderPgm(), "material.kDiffuse", BVH.GetBVHLvColors()[bv->depth]);
                        }
